...
const LANG_COLORS: Record<string, string> = {
  ...
  Go: '#00ADD8', Java: '#b07219', 'C++': '#f34b7d', C: '#888', Ruby: '#701516',
...
/* ─────────────────── STAT CARD ─────────────────── */
function StatCard({ label, value, color, barW = 55 }: { label: string; value: number; color: string; barW?: number }) {
  return (
    <div style={{ padding: '10px 12px 8px', background: 'rgba(13,17,23,0.8)', border: '1px solid rgba(0,229,255,0.1)', borderRadius: 8, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
...
function RepoCard({ repo, idx }: { repo: GithubRepo; idx: number }) {
  ...
  return (
    <div
      ...
      style={{ display: 'flex', flexDirection: 'column', padding: '10px 12px', gap: 6, background: hov ? 'rgba(0,229,255,0.03)' : 'rgba(13,17,23,0.7)', border: `1px solid ${hov ? 'rgba(0,229,255,0.3)' : 'rgba(0,229,255,0.1)'}`, borderRadius: 8, transition: 'all 0.18s', transform: hov ? 'translateY(-1px)' : 'none', overflow: 'hidden', height: '100%', boxSizing: 'border-box' }}
    >
      {/* header */}
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <span style={CS.repoStat}>★ {fmt(repo.stargazers_count)}</span>
        <span style={CS.repoStat}>⑂ {fmt(repo.forks_count)}</span>
        <span style={{ ...CS.repoStat, marginLeft: 'auto' }}>⊙ {timeAgo(repo.updated_at)}</span>
      </div>
      {/* analyze btn */}
...
export default function Dashboard() {
  ...
  // Build activity feed from repos
  const now = new Date()
      ...
      <nav style={{ height: 50, flexShrink: 0, position: 'relative', zIndex: 100, display: 'flex', alignItems: 'center', gap: 14, padding: '0 20px', background: 'rgba(5,5,5,0.95)', borderBottom: '1px solid rgba(0,229,255,0.08)', backdropFilter: 'blur(14px)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button onClick={logout} disabled={loggingOut} title="Sign out" style={{ width: 26, height: 26, background: 'transparent', border: '1px solid rgba(255,68,102,0.25)', borderRadius: 6, color: '#ff4466', fontSize: 10, cursor: 'pointer' }}>✕</button>
          </div>
        </div>
      </nav>
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: '230px 1fr 200px', gap: 10, padding: '10px 14px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flexShrink: 0, padding: '14px', background: 'rgba(13,17,23,0.8)', border: '1px solid rgba(0,229,255,0.1)', borderRadius: 10, backdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {user.location && <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><span style={{ color: 'rgba(0,229,255,0.4)', fontSize: 10 }}>◎</span><span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: '#7d8590', letterSpacing: '0.06em' }}>{user.location.toUpperCase()}</span></div>}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: 2, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 6, overflow: 'hidden' }}>
            <div style={CS.secTitle}><span style={{ color: '#00E5FF' }}>◎</span> QUICK ACTIONS</div>
            {[
              { icon: '＋', bg: 'rgba(0,255,136,0.1)', ic: '#00ff88', title: 'Analyze Repository', desc: 'Scan a GitHub URL', href: '/' },
              { icon: '◎', bg: 'rgba(123,97,255,0.1)', ic: '#7B61FF', title: 'Explore Planet', desc: 'Top AI architectures', href: user.html_url },
              { icon: '★', bg: 'rgba(255,165,0,0.1)', ic: '#ff9500', title: 'Starred Repos', desc: 'Your starred repos', href: `${user.html_url}?tab=stars` },
            ].map(a => (
              <a key={a.title} href={a.href} target={a.href.startsWith('http') ? '_blank' : '_self'} rel="noreferrer"
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(0,229,255,0.28)' }}
